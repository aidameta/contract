...
contract CrystalCoin is ERC20(), Ownable {
    ...
    mapping(uint => stakeRate) private _stakeRateMap;
    uint private stakeRateCount;
    ...
    function registerContractAddress(address contractAddress) public {
        ...
        if (!lend.isValue) {
            ...
            lendAddress[contractAddress] = lend;
            lendAddressIndex[currentLendAddressIndex++] = contractAddress;
        }
    }
    ...
    function transferCrystalCoin(address to, uint256 amount) public returns (bool) {
        ...
        if (lend.isValue) {
            ...
            lendAddress[msg.sender].val += amount;
            return true;
        } 
        else {
            return false;
    ...
    function regainCrystalCoin(address from, uint256 amount) public returns (bool) {
        lendData memory lend = lendAddress[msg.sender];
        if (lend.isValue) {
            require(lendAddress[msg.sender].val >= amount, 'money not enought');
    ...
    function getLendData() public view returns (lendData[] memory lends) {
        lends = new lendData[](currentLendAddressIndex);
        for (uint i = 0 ; i < currentLendAddressIndex; i++) {
            address add = lendAddressIndex[i];
            lendData memory lend = lendAddress[add];
            lends[i] = lend;
        }
    ...
    function pushArrayValue(uint[] memory arr, uint v) private pure returns (uint[] memory) {
        if (arr.length <= 0) {
        ...
        {
            uint[] memory res = new uint[](1 + arr.length);
            uint i=0;
    ...
    function removeArrayValue(uint[] memory arr, uint v) private pure returns (uint[] memory) {
        uint findCount = 0;
        for (uint i = 0 ; i < arr.length; i++) {
            if (arr[i] == v) findCount++;
        }

        uint[] memory result = new uint[](arr.length - findCount);
        uint k = 0;
        for (uint i = 0 ; i < arr.length; i++) {
            if (arr[i] != v) result[k++] = arr[i];
        } 

        return result;
    }

    function setStakeRate(uint id, uint time, uint rate) public  {
        require(msg.sender == managerLeadAddress, 'privage reject');
        stakeRate memory _rateData = _stakeRateMap[id];
        if (_rateData.isValue) {
            _rateData.stakeTime = time;
            _rateData.rate = rate;
            _stakeRateMap[id] = _rateData;
        }
        else {
            _rateData = stakeRate({
                id: id,
            ...
            _stakeRateMap[id] = _rateData;
            if (stakeRateCount < id) {
                stakeRateCount = id;
            }
        }
    }

    function getStakeRates() public view returns(stakeRate[] memory) {
        uint count = 0;
        for (uint i = 0 ; i <= stakeRateCount; i++) {
            stakeRate memory tmp = _stakeRateMap[i];
            if (tmp.isValue) count++;    
        }
        stakeRate[] memory results = new stakeRate[](count);
        uint k = 0;
        for (uint i = 0 ; i <= stakeRateCount; i++) {
            stakeRate memory tmp = _stakeRateMap[i];
            if (tmp.isValue) {
    ...
    function getStakeRate(uint time) public view returns (uint) {
        uint rate = 0;
        for (uint i = 0 ; i <= stakeRateCount; i++) {
            stakeRate memory tmp = _stakeRateMap[i];
            if (tmp.isValue) {
                if (tmp.stakeTime >= time && tmp.rate >= rate) {
                    rate = tmp.rate;
                }
            }
        }
    ...
    function getUserStake(address from) public view returns (stakeData[] memory) {
        uint[] memory idList = userStakeMap[from];
        if (idList.length >= 1) {
            stakeData[] memory result = new stakeData[](idList.length);
            for(uint i = 0 ; i < idList.length; i++) {
                result[i] = stakeMap[idList[i]];
            }
            return result;
        }
        else {
            return new stakeData[](0);
        }
    }
